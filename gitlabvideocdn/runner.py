...
def run_conversion(data: dict = None) -> int:
    """GitLab CI/CD ortamında video dönüştürme işlemini baştan sona icra eder."""
    ...
    video_id = data.get("video_id") or (os.environ.get("CI_PIPELINE_ID") and f"gl_{os.environ.get('CI_PIPELINE_ID')}") or str(int(start_time))[-8:]
    ...
    # Çalışma klasörü
    work_dir = f"/tmp/convert_{video_id}"
    ...
    try:
        ...
        print(f"[{video_id}] 1. Aşama Başlatılıyor: İndirme (Kullanıcı: {username}, ID: {custom_id})")
        ...
        for p in DEFAULT_PROFILES:
            p_name = p["name"]
            # Eğer belirli kaliteler istendiyse filtrele, istenmediyse orijinal çözünürlüğe eşit veya küçükleri al
            if requested_list:
        ...
        if not active_profiles:
            active_profiles = [DEFAULT_PROFILES[0]]  # En az 360p
        ...
        # FFmpeg işlemini çalıştır ve ilerlemeyi takip et
        ...
        for line in proc.stdout:
            ...
            # Stagnation watchdog kontrolü (45 saniye donma engeli)
        ...
        if not storage_host or not storage_user or not storage_pass:
            ...
            # Toplam yükleme boyutunu hesapla
            total_up_bytes = 0
            ...
            # SSH ControlMaster soketini kapat
            try:
        ...
        perf_stats = build_accumulated_perf_stats(work_dir, start_time)
        credit_info = calculate_gitlab_credits(total_elapsed, data)
        ...
        tracker.send_event(step="completed", progress=100, status="completed", extra={
            ...
            "credit_unit": credit_info.get("credit_unit", "minutes"),
