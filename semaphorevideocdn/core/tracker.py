...
def check_and_raise_cancellation(video_id: str, work_dir: str = None):
    ...
    """
    target_dir = work_dir or f"/tmp/semaphore_convert_{video_id}"
...
def setup_cancellation_and_timeout_handlers(tracker: ProgressTracker, start_time: float, work_dir: str = None):
    """
    Semaphore CI Runner'ın veya sistemin tetiklediği SIGTERM, SIGINT sinyallerini yakalar
    ve senkron 'cancelled' / 'failed' webhook'u göndererek temizlik yapar.
    """
    def handle_signal(sig, frame):
        sig_name = (
            "SIGTERM (Semaphore CI Cancel/Timeout)" if sig == signal.SIGTERM
        ...
        tracker.send_event(
            ...
            extra={
                ...
                "message": "Dönüştürme işlemi Semaphore CI üzerinden iptal edildi veya zaman aşımına uğradı.",
