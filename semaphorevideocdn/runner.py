...
def parse_payload() -> dict:
    """Komut satırından veya Semaphore CI ortam değişkenlerinden dönüştürme parametrelerini ayrıştırır."""
    parser = argparse.ArgumentParser(description="Semaphore CI Video HLS Converter Runner")
    ...
    # Bireysel Semaphore CI ortam değişkenlerini oku ve eksik alanları doldur / üzerine yaz
    env_mappings = [
        ("video_url", ["VIDEO_URL"]),
...
def run_conversion(data: dict = None) -> int:
    """Semaphore CI ortamında video dönüştürme işlemini baştan sona icra eder."""
    ...
    # Semaphore CI workflow veya pipeline ID'sini video_id fallback olarak kullan
    sem_wf_id = os.environ.get("SEMAPHORE_WORKFLOW_ID") or os.environ.get("SEMAPHORE_PIPELINE_ID") or os.environ.get("SEMAPHORE_JOB_ID")
    video_id = data.get("video_id") or (sem_wf_id and f"sem_{sem_wf_id[:8]}") or str(int(start_time))[-8:]
    ...
    # Çalışma klasörü
    work_dir = f"/tmp/semaphore_convert_{video_id}"
    ...
    try:
        ...
        print(f"[{video_id}] 1. Aşama Başlatılıyor: İndirme (Kullanıcı: {username}, ID: {custom_id}, Platform: Semaphore CI)")
        ...
        perf_stats = build_accumulated_perf_stats(work_dir, start_time)
        credit_info = calculate_semaphore_credits(total_elapsed, data)
